...
by (two-tailed) *t*-test, *p* = 0.04. You decide to follow up their 
work.

## Question 2 (continued) {.smaller}
...
- First, make a guess, then calculate it (adapted from Dienes, 2008, p. 64).  

## Question 2 (Hints) {.smaller}

- You can use the pwr package, specifically the `pwr.t.test()` function. You should aim for decent power (typically power = 0.8).
- You can calculate Cohen’s *d* for a t-test like this from *t*: $d=\frac{2t}{\sqrt{df}}$ where *t* is the t-value and *df* is the degrees of freedom of the test.
- What if you don't have t-value? You can find the t-value from the reported p-value ($0.04$) using the qt() function in R.
...
# Question 3 {.smaller}
- Presume that like Miller and Jones you ran 30 participants in each group. You obtain a non-significant result in the same direction, *t* = 1.24 (p = 0.22). 
- Should you:
  - try to find an explanation for the difference between the two studies?  
  - regard the Smith and Jones result as now thrown into doubt; you should reduce your confidence in the effectiveness of their method for overcoming prejudice?  
...
## Answer 2: Calculate *t*-value from *p*-value {.smaller}
- With the degrees of freedom, we can get the t-value from the p-value (.04)
- Important: Since this is a two-tailed test with $p=.04$, there is $2.5\%$ probability in the tails, meaning $.02$ (2%) in each tail. 
- So we need to find the t-value that corresponds to $.02$ in the upper tail with $58$ degrees of freedom.
- In R, we can use the `qt()` function to find this: `qt(.02,58)` gives us `r qt(.02,58)`
- The question does not give a sign for the effect (positive/negative). We could also do `qt(.98,58)` or `qt(.02,58, lower.tail = FALSE)`, which would give us the positive t-value. For our power considerations, it doesn't matter which one we use.
        
## Answer 2: Calculate power {.scrollable}
...
- Now use the pwr package to calculate the required sample size for a power of .8
```{r}
#| label: power-calculation-replication
#| echo: true

effect_size <- 4.2 / sqrt(58)
pwr.t.test(
...
- A non-significant result in the same direction as the effect you were trying to replicate doesn't tell you much
    - It is definitely not (necessarily) evidence in favor of the null hypothesis!
    - Remember, **you cannot accept the null hypothesis**. So don't interpret your results as if one study had found an effect and the other didn't.
...
- If you were tempted to say "can't tell" or "evidence in favor of the null", that's not correct
